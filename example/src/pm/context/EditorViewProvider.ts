...
export class EditorViewProvider {
  ...
  replaceDoc(rawValue: Object) {
    try {
      const doc = this.view.state.schema.nodeFromJSON(rawValue)
      this.view.dispatch(this.view.state.tr.replaceWith(0, this.view.state.doc.nodeSize - 2, doc))
    } catch (err) {
      console.error(err)
    }
  }
